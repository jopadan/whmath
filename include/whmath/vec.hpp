...
/*
	template<typename... A,
		typename = std::enable_if_t<sizeof...(A) < N>,
		typename = std::enable_if_t<(std::is_convertible_v<std::decay_t<A>, T> && ...)>>
			constexpr vec<T, N> vector_product( A&&... args )
			{
...
*/
						 ...
						 inline constexpr vec<T, N> identity( A&&... args )
						 {
						 return { static_cast<T>(args ? 1 : 0)... };
						 }
						 inline constexpr vec<T, N> identity( size_t n )
						 {
						 vec<T, N> dst = { 0 };
						 dst[n % N] = 1;
						 return dst;
